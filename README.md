...
Tauri macOS development app：`scripts/dev/tauri_dev_app.py` 只会 stage 允许的共享 core 并构建 `.app`；不创建 DMG、更新归档、签名、公证、Release 或 Feed。L9 已对一个 arm64 app 执行一次隔离启动和正常退出烟测。开发 profile 的 `INVOICE_HUB_DEV_STATE_ROOT` 必须显式提供一个已存在的绝对目录；host 会 canonicalize 它并拒绝其位于 bundle/core 内或包住 bundle/core 的情况，且不会将变量传给 Python child；release profile、缺失或相对路径会 fail closed。
