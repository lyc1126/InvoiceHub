...
hosted host-lock 竞争的 busy 响应在返回前不写 `updates.checked` 事件，也不等待 SQLite；正常成功或非竞争检查仍保留原有更新检查事件。Tauri `setup` 在启动 owned backend 后先完成 tray 与已选 surface 的可失败初始化；任何失败均在返回原 setup error 前通过既有 keep-monitor shutdown 和必要的 kill+wait 收束该 child，因此不会依赖尚未注册的退出 handler 或进程 Drop。
