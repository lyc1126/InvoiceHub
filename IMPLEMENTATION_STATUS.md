...
- internal-alpha 发行准备已落地到独立脚本：`scripts/dev/tauri_alpha_release.py` 从精确 Git snapshot 复制 allowlist core，校验并嵌入 Python 3.14.6 arm64 runtime，生成 schema-3 host manifest、launcher/build/package/runtime manifest 和 schema-4 receipt；`scripts/dev/verify_tauri_alpha.py` 对 App、可选 DMG、哈希、布局、平台污染和 ad-hoc 模式做 fail-closed 校验。当前只完成构建链和依赖准备，真实 alpha `.app/.dmg` 构建与启动烟测仍待本轮执行。
