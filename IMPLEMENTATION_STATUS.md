...
- 本仓库已以单一、脱敏的本地根提交建立候选 `main`。旧的私有提交图、验证记录、二进制包和 Tag 不属于公开历史，也不会作为 Release 资产上传。
- 首个公开开发版本为 `0.3.0-alpha.1`；任何公开二进制都必须从脱敏图上的新版本、新 Tag 和新发布证据构建。
- 历史净化的范围、私有备份和验证门槛见 [执行记录](docs/release/HISTORY_SANITIZATION_EXECUTION.md)。通过全部门槛前，仓库保持 private，不能创建 Release、更新 Feed 或公开后 Tauri 分支。
...
- 公开前只运行一次候选内容审计和一次保留 refs 全量审计。真实命中即停止公开流程，先隔离或替换，不用刷新式重跑掩盖问题。
