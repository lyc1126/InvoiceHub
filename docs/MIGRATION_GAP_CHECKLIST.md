...
- [ ] P1-Q：clean-commit 外部 AppleScript quit 绕过 shutdown POST 并留下 `server_state=ready`，已撤回 L9 的有序退出结论。必须由自定义 macOS 应用菜单 Quit/Cmd-Q 与 tray 共用 `app.exit(0)`，禁止 predefined Quit，并在隔离 state root 重验 shutdown POST、stopped state、child/PID/端口清理后才能创建 Draft PR；Force Quit、SIGKILL 等不属于可拦截承诺。
