...
| `src-tauri/src/main.rs` foundation guard | 存在 Tauri 配置或宽泛 `tauri = "2"` 依赖声明，不等于有可复现的 Rust host；允许它启动会误连旧 localhost 或掩盖端口所有权缺失 | 在精确 crate 版本、Cargo lock 和生命周期实现之前固定非零退出；不得换端口、接入未知实例或伪造桌面能力 | `tests/test_tauri_foundation.py` 静态固定-origin/guard 回归 |
