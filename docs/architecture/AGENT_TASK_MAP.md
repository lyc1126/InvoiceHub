...
| 真实验收 | L9 已覆盖 development app 的 fixed-port owned backend、health/background、首页/静态资源、desktop 默认和退出清理；仍需 owned/external、browser、NSOpenPanel、tray/单实例、预览/打印、签名/notary/staple、quarantine、首次目录授权、Sparkle 旧版到新版且 monitor 恢复 |
...
| 最低自动化 | 当前先覆盖 hosted fresh Feed `200` approval 与 cache/ETag/`304` rejection、host-check immediate-busy/approval-retention、non-host check bypass、install-lock immediate-error/approval-retention/no-second-RPC、candidate 主动过期/一次性清除、fail-closed install、空 install body/脱敏 503、tray keep-monitor shutdown 与 state-root/provenance 门禁；下载验签、monitor stop/recheck、安装/restart 只在 recovery/relaunch coordinator 实现后进入同一专题的最小测试 |
