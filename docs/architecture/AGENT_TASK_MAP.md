...
| 真实验收 | L9 已覆盖 development app 的 fixed-port owned backend、health/background、首页/静态资源和 desktop 默认；原退出清理结论已被 clean-commit 外部 quit 样本撤回，P1-Q 必须用自定义应用菜单/Cmd-Q 重验 shutdown POST、stopped state、child/PID/端口清理；仍需 owned/external、browser、NSOpenPanel、tray/单实例、预览/打印、签名/notary/staple、quarantine、首次目录授权、Sparkle 旧版到新版且 monitor 恢复 |
