...
> 当前边界：候选树、Git 对象和托管面验证通过后，`v0.3.0-alpha.1` Tauri 2 才从公开后的 `main` 开发。
...
| 高风险提醒 | 退休预公开包、receipt 和 Tag 不得重打、复用或上传。未完成经批准的历史净化不得公开、上传 Release 或创建公开后 Tauri 线。每项实验先写假设、决策、最小样本和停止条件；相同失败机制只取一个代表样本。Tauri 不重写业务核心，未知 `127.0.0.1:8766` 占用必须失败，Host RPC token 不得暴露网页，安装前 monitor 停止失败或用户取消都不得改变运行状态。 |
